---
title:          LEGO':' Leveled Language Gaussian Splatting
date:           2026-06-18 00:01:00 +0800
selected:       true
pub:            "European Conference on Computer Vision (ECCV)"
pub_date:       "2026"
pub_last:       "3D Level-wise Open-vocabulary Understanding"
abstract: >-
  LEGO self-adaptively re-grades volatile multi-view SAM granularities into a unified, open-vocabulary, 3D-consistent hierarchy, such as the "flowerpot - bouquet - bud - petal" lineage.
  
cover_video:          assets/images/covers/lego.mp4
authors:
  - Yuning Peng
  - Haiping Wang†
  - Yuan Liu
  - Yipeng Lu
  - Zhen Dong
  - Bisheng Yang
links:
  Paper: 
  Code: https://github.com/WHU-USI3DV/LEGO
  Project Page: https://pz0826.github.io/LEGO-Webpage/
---